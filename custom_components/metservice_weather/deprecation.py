...
# Extra sentence appended to the removed-entity repair's replacement text,
# keyed by replacement key, for removals where the single replacement
...
# Values must start with a leading space (appended flush after the
# replacement display name); keys without an entry get "".
_REPLACEMENT_DETAIL_HINTS: dict[str, str] = {
    "warning_level": (
        " — for the full warning text, use the Warning details sensor: "
...
def _removed_entity_replacement_text(key: str) -> str:
    ...
    Known replacements get their display name plus any per-key detail hint
    (GH issue #32: weather_warnings migrators need pointing at Warning
    details for the full text, not just the Warnings enum).
    """
    new_key = _DEPRECATED_SENSOR_REPLACEMENTS_LOWER.get(key.lower())
    if new_key is None:
        return _GENERIC_REMOVED_REPLACEMENT_FALLBACK
    return _replacement_display_name(new_key) + _REPLACEMENT_DETAIL_HINTS.get(
        new_key, ""
    )
